...
const TaskPlanner = () => {
  ...
  const [modalData, setModalData] = useState({ name: '', category: 'To Do', isEdit: false, taskId: null });
  const [draggedTask, setDraggedTask] = useState(null);
  const [resizingTask, setResizingTask] = useState(null);
  const [resizeType, setResizeType] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [categoryFilters, setCategoryFilters] = useState(Object.keys(CATEGORIES));
  ...
  const handleDayMouseDown = (date, e) => {
    if (!date) return;
    e.preventDefault();
    setIsSelecting(true);
  ...
  const handleDayMouseEnter = (date) => {
    ...
    const days = [];
    const currentTime = Math.min(startTime, endTime);
  ...
  const handleTaskSubmit = () => {
    ...
    if (modalData.isEdit) {
      setTasks(tasks.map(task => 
        task.id === modalData.taskId 
          ? { ...task, name: modalData.name, category: modalData.category }
          : task
      ));
    } else {
      const newTask = {
  ...
  // Task drag and drop
  const handleTaskMouseDown = (task, e, type = 'move') => {
    e.stopPropagation();
  ...
  const handleTaskDrop = (targetDate) => {
  ...
  const handleTaskResize = (targetDate) => {
  ...
  const handleDayMouseUpForDrop = (date) => {
    if (draggedTask) {
      handleTaskDrop(date);
  ...
  const getFilteredTasks = () => {
    return tasks.filter(task => {
      ...
      if (durationFilter !== 'all') {
        ...
        const diffTime = Math.abs(taskStart - today);
  ...
  const getTasksStartingOnDay = (date) => {
    if (!date) return [];
    const dateStr = formatDate(date);
  ...
  const getTaskSpanInfo = (task, startDate) => {
  ...
  const handleSearchAndNavigate = (searchValue) => {
    setSearchTerm(searchValue);
    if (searchValue.trim()) {
      // Find first matching task
      const matchingTask = tasks.find(task => 
        task.name.toLowerCase().includes(searchValue.trim().toLowerCase())
      );
      if (matchingTask) {
  ...
  const getTaskPosition = (task, dayTasks, date) => {
    const taskIndex = dayTasks.indexOf(task);
    const isFirstDay = formatDate(date) === task.startDate;
    const isLastDay = formatDate(date) === task.endDate;
                ...
                {date && (
                  <>
                    <div className={`text-sm font-semibold p-1 ${
                      isToday(date) ? 'text-blue-700 bg-blue-100 rounded-full w-6 h-6 flex items-center justify-center m-1' : 'text-gray-600 m-1'
                    }`}>
                      {date.getDate()}
                    </div>
                            ...
                            {hoveredTask?.id === task.id && (
                              <div className="absolute z-50 top-full left-0 mt-2 p-3 bg-gray-900 text-white text-xs rounded-lg shadow-xl whitespace-nowrap border border-gray-700">
                                ...
                                <div className="text-gray-400 text-xs">
                                  {parseDate(task.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - {parseDate(task.endDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                                </div>
                                <div className="text-gray-400 text-xs">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-96 max-w-md mx-4">
              <h3 className="text-lg font-semibold mb-4">
                {modalData.isEdit ? 'Edit Task' : 'Create Task'}
              </h3>
              ...
              <div className="space-y-4">
                ...
                {!modalData.isEdit && selectedDays.length > 0 && (
                  <div className="text-sm text-gray-600">
                    Duration: {selectedDays[0].toLocaleDateString()} - {selectedDays[selectedDays.length - 1].toLocaleDateString()}
